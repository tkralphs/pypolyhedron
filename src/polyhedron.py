...
def _as_array(x, dtype=float):
    """Convert input to a NumPy array with safe defaults (NumPy 1/2 compatible)."""
    if x is None:
        return None
...
class Polyhedron(object):
    ...
    def __str__(self):
        ...
        for i in range(len(self.generators)):
            gi = np.asarray(self.generators[i])
            ret = '%s\t%s[%s]=%s\n' % (
                ret,
                {0: 'ray', 1: 'vertex'}[self.is_vertex[i]],
                i,
                gi.tolist()
            )
            ...
            for i in range(len(self.inc)):  # i-th generator
                if self.is_vertex[i]:
                    for j in self.inc[i]:  # j-th normal
                        ...
                        ret = '%s (%s == %s)\n' % (
                            ret,
                            scalmul(self.A[j], self.generators[i]),
                            self.b[j]
                        )
                else:
                    for j in self.inc[i]:  # j-th normal
...
class Hrep(Polyhedron):
    def __init__(self, A, b=None, eq=None):
        A = _as_array(A)
        b = _as_array(b)
        eq = None if eq is None else list(eq)
